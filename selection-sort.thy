...
lemma p_100 [simp]: "m = Min(set (x#xs)) \<Longrightarrow> m \<in> set (x#xs)"
proof(induct xs arbitrary: x)
  case Nil
  then show ?case by simp
next
  case (Cons a xs)
  then show ?case using eq_Min_iff by blast
qed
...
termination by (meson "termination" in_measure p_100 p_5 wf_measure)
...
next
  case (2 x xs)
  then show ?case by (metis "100" mset.simps(1) mset.simps(2) mset_append mset_rev p_100 rev.simps(2) selection_sort.simps(2))
qed
